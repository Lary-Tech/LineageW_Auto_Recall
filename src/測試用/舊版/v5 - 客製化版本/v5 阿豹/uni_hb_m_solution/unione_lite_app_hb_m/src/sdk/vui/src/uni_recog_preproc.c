...
static uni_s32 _assign_nlu_type(cJSON *jnlu) {
  ...
  if (uni_strcmp(cmd, "startStudy") == 0
    || uni_strcmp(cmd, "startStuCmd") == 0
    || uni_strcmp(cmd, "startStuWakeup") == 0
    || uni_strcmp(cmd, "clearStuWakeup") == 0
    || uni_strcmp(cmd, "clearStuCmd") == 0
    || uni_strcmp(cmd, "clearStudy") == 0) {
    type = ID(VUI_APP_STUDY_EVENT);
    goto L_END;
  }
L_END:
  if (cmd) {
...
static void _study_result_event_free_handler(EventContent *event_content) {
  if (NULL != event_content->info) {
...
static Result _lasr_result_preproc(Event *event, float std_thresh,
  ...
  /* 2. parse command to get nlu or is study result*/
  if (uni_strstr(event->content.extend_info, "study") != NULL) {
    ...
    uni_u32 *command_hash = uni_malloc(sizeof(uni_u32));
    if (command_hash == NULL) {
      LOGE(RECOG_PREPROC_TAG, "malloc command_hash size %d failed", sizeof(uni_u32));
      goto err_exit;
    }
    *command_hash = uni_hash(command);
    event->type = ID(VUI_APP_STUDY_RESULT_EVENT);
    event->content.info = (void *)command_hash;
    event->free_handler = _study_result_event_free_handler;
    return E_OK;
  } else if (NULL == (jnlu = NluParseLasr(command))) {
    goto err_exit;
  }
